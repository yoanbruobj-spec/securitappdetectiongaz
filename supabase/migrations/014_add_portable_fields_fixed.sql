...
DROP POLICY IF EXISTS "Utilisateurs authentifiés peuvent lire portables" ON public.portables;
CREATE POLICY "Utilisateurs authentifiés peuvent lire portables"
    ON public.portables FOR SELECT
    USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Techniciens peuvent gérer portables" ON public.portables;
